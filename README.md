# LanguageSwitcher (AutoHotkey v2)
...
### `Ctrl + Break` (обычно это `Ctrl + Pause`)
...
- `Pause` — исправить **всю строку от начала до курсора** и сразу продолжить печатать на правильном языке.
- `Shift + Pause` — исправить **то, что уже выделил**, без автодобора текста.
...
- Windows
- Файлы:
  - `LanguageSwitcher_Main.ahk` — основной скрипт.
  - `LanguageSwitcher_UI.ahk` — модуль интерфейса (подключается через `#Include`).
